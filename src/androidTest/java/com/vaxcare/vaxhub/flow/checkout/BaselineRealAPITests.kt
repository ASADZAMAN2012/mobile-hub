...
@LargeTest
class BaselineRealAPITests : TestsBase() {
    companion object {
    ...
    @Test
    fun riskFreeCheckoutWithNewAndExistingDose_test() {
        val testProduct1 = TestProducts.Varicella
        val testProduct2 = TestProducts.IPV
        var appointmentId = ""
        ...
        registerMockServerDispatcher(
            RPRDTestsDispatcher(
                ...
                testDirectory = "riskFreeCheckoutWithNewAndExistingDose_test"
            )
        )
        ...
        appointmentListScreen {
            ...
            verifyAppointmentIconAndTags(
                ...
                stockPillList = listOf(TestStockPill.PrivateStockPill)
            )
            tapFirstAppointmentWithPatient(testPatient)
        }
        Thread.sleep(6000)
        checkOutUtil.addNewLotUsingMagnifyingGlassOnViewPort(testProduct1)
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        checkOutUtil.performDoseActions(AddDose(testProduct2))
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        checkOutUtil.waitForElementAndClick("Intramuscular")
    ...
    @Test
    fun completeVFCCheckoutValidatePostCheckoutStatus_test() {
        val testPartner = TestPartners.RprdCovidPartner
        ...
        registerMockServerDispatcher(
            RPRDTestsDispatcher(
                useCases = checkoutUseCases,
                clinicId = testPartner.clinicID.toLong(),
                testDirectory = "vfcCheckout_test"
            )
        )
    ...
    @Test
    fun editRemoveDoseFromChecked_test() {
        val testPatient = TestPatients.RiskFreePatientForCheckout()
        val testPartner = TestPartners.RprdCovidPartner
        val testProductsList = arrayListOf(TestProducts.Varicella, TestProducts.Adacel)

        registerMockServerDispatcher(
            RPRDTestsDispatcher(
                useCases = checkoutUseCases,
                clinicId = testPartner.clinicID.toLong(),
                testDirectory = "editRemoveDoseFromCheckedOut_test"
            )
        )
        ...
        checkOutUtil.clickFirstElementInAppointmentListByPatientName(testPatient)
        Thread.sleep(5000)
        checkOutUtil.performDoseActions(AddDose(testProductsList[0]))
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        checkOutUtil.waitForElementAndClick("Keep Stock")
        checkOutUtil.performDoseActions(AddDose(testProductsList[1]))
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        checkOutUtil.tapArrowToCheckoutSummary()
        reviewDosesScreen {
            selectUnorderedReasonDoseNotAppearingForAll()
        ...
        appointmentListUtil.checkReloadForAppointmentListIfEligibilityNotBack(testPatient, 3)
        appointmentListUtil.tapFirstElementInAppointmentListByPatientName(testPatient)
        checkOutUtil.performDoseActions(RemoveDose(testProductsList[1]))
        checkOutUtil.waitForElementAndClick("Remove Dose")
        reviewDosesScreen {
            tapArrowToCheckoutSummary()
            selectUnorderedReasonDoseNotAppearingForAll()
            tapArrowToCheckoutSummary()
    ...
    @Test
    fun addNewMaternalRSVLotFromCheckout_test() {
        val testPartner = TestPartners.RprdCovidPartner
        val testPatient = TestPatients.PregnantPatient()
        val testProduct1 = TestProducts.RSV

        registerMockServerDispatcher(
            RPRDTestsDispatcher(
                useCases = checkoutUseCases,
                clinicId = testPartner.clinicID.toLong(),
                testDirectory = "checkOutPregnantRSV_test"
            )
        )
        ...
        checkOutUtil.addNewLotUsingMagnifyingGlassOnViewPort(testProduct1)
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        checkOutUtil.pregnantWeeks("32 weeks")
        checkOutUtil.tapArrowToCheckoutSummary()
        reviewDosesScreen {
            selectUnorderedReasonDoseNotAppearingForAll()
            tapArrowToCheckoutSummary()
    ...
    fun adjustVisitDate_test() {
        ...
        calendarUtil.clickCalendarIcon()
        repeat(3) {
            IntegrationUtil.waitForOperationComplete(1)
            calendarUtil.clickCalendarBackButton()
            calendarUtil.waitForLoadingGone()
        }

        repeat(6) {
            IntegrationUtil.waitForOperationComplete(1)
            calendarUtil.clickCalendarForwardButton()
            calendarUtil.waitForLoadingGone()
        }
    ...
    //CORE-21501
    @Test
    fun searchForPatientAndAddAppointment_test() {
        val testPartner = TestPartners.RprdCovidPartner
        ...
        registerMockServerDispatcher(
            RPRDTestsDispatcher(
                useCases = checkoutUseCases,
                clinicId = testPartner.clinicID.toLong(),
                testDirectory = "searchForPatient_test"
            )
        )
        ...
        Thread.sleep(5000)
        homeScreenUtil.tapHomeScreenAndPinIn(testPartner)
        checkOutUtil.createAndSearchPatientAppointment(testPartner,testPatient)
        Thread.sleep(60000)
        checkOutUtil.performDoseActions(AddDose(testProduct))
        checkOutUtil.waitForElementAndClick("Keep Stock")
        checkOutUtil.tapArrowToCheckoutSummary()
    ...
    @Test
    fun splitPartnerBillVisitAndValidateSplitVisit_test() {
        val testPatient = TestPatients.RiskFreePatientForCheckout()
        val testPartner = TestPartners.RprdCovidPartner
        val testProductsList = arrayListOf(TestProducts.Varicella, TestProducts.Dtap)

        registerMockServerDispatcher(
            RPRDTestsDispatcher(
                useCases = checkoutUseCases,
                clinicId = testPartner.clinicID.toLong(),
                testDirectory = "checkOutSplitPartnerBill_test"
            )
        )
        ...
        checkOutUtil.performDoseActions(AddDose(testProductsList[0]))
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        IntegrationUtil.waitForElementToAppearAndClick(onView(withText("Keep Stock")))
        checkOutUtil.performDoseActions(AddDose(testProductsList[1]))
        checkOutUtil.promptUnorderedDoseDialogToSelect("Yes, Keep Dose")
        IntegrationUtil.waitForElementToAppearAndClick(onView(withText("Partner Bill")))
        checkOutUtil.tapArrowToCheckoutSummary()
        reviewDosesScreen {
            selectUnorderedReasonDoseNotAppearingForAll()
            tapArrowToCheckoutSummary()
